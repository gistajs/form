...
Required manual approval for production Atlas schema applies

- The `atlas:prod` script no longer passes `--auto-approve`, so production schema changes prompt for confirmation before applying.
...
Fixed starter builds after the React Email 6 upgrade

- The shared auth email rendering files shipped with the correct `react-email` imports, so materialized starter typechecks pass again.
